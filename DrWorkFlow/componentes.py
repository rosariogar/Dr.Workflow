...
def load_data(file_path):
    ...
    try:
        ...
        print("Por favor, verifica la ruta o el formato del archivo CSV e intenta nuevamente.")
        return None
...
def autoML(df, target_col, test_size=0.2, random_state=42):
    ...
    if best_model is not None and num_classes == 2:
        ...
        plt.title(f'Curva ROC de {best_model_name}')
        plt.legend(loc='lower right')
        plt.savefig(f'roc_curve_best_model_{timestamp}_{target_col}.png')  # Guardar imagen de la curva ROC para el mejor modelo
        plt.close()
